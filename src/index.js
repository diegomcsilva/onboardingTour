...
function checkIsPageAllowed(params) {
  var pageIsAllowed = false;

  pagesAllowed.forEach(function(item) {
    if(window.location.pathname.indexOf(item) > -1) {
      pageIsAllowed = true;
    }
  });

  return pageIsAllowed && !checkDefinedInCookie();
}

function createBgEl() {
  document.querySelector('body').insertAdjacentHTML(
      "afterbegin",
      '<div class="bg-shadow isActive"></div>'
    );
}
...
function changeStep(data) {
  ...
  if(data.position && !isMobile) {
    ...
    if(isMobile) {
      ...
      var positionOnboardingStep = 102;
      onboardingStepIntoEl.style.top = positionOnboardingStep + 'px';
    } else {
      var offsetExtra = 25;
      var offsetTotal = clientHeight + offsetTop + offsetExtra;
      var newPositionScroll = onboardingActiveEl.getBoundingClientRect().top + window.scrollY - offsetTotal;
...
function setCookieNotShowTour(params) {
  setCookie("notShowTour", [getCookie('notShowTour'), window.location.pathname]); 
};
...
function checkDefinedInCookie() {
  var currentCookie = getCookie('notShowTour');
  return currentCookie.indexOf(window.location.pathname) > -1;
};

function start() {
  document.addEventListener("DOMContentLoaded", function () {
    if (checkIsPageAllowed()) {
      createBgEl();
      startSteps();  
    }
  })
  ...
  module.exports = { start, defaultImage }